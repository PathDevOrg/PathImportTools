...
import { type AuraRows, type ConversionResult } from "@aura-importer/converter";
...
export async function createAuraDatabaseOutput(
  ...
  if (target.saveHandle && !canStreamToSaveHandle) {
    throw new Error("This browser cannot stream large database files. Use a Chromium browser with OPFS enabled, or convert a smaller backup.");
  }
  ...
  try {
    ...
    db.exec("BEGIN");
    try {
      insertRows(db, result.rows, onProgress);
...
function insertRows(db: SqliteDatabase, rows: AuraRows, onProgress: (progress: DatabaseProgress) => void): void {
  ...
  const progress = (table: keyof AuraRows): void => {
    if (completed === total || completed % 500 === 0) {
      onProgress({ phase: "write", message: `Writing ${table}`, completed, total });
    }
  };
  try {
    ...
    const insertTableRows = (plan: { table: keyof AuraRows; columns: string[] }, tableRows: Array<Record<string, unknown>>): void => {
      for (const row of tableRows) {
        insertRow(plan, row);
      }
    };

    insertTableRows(tablePlan("pois"), rows.pois as Array<Record<string, unknown>>);
    ...
    for (const plan of insertPlans) {
      ...
      insertTableRows(plan, rows[plan.table] as Array<Record<string, unknown>>);
    }
  } finally {
