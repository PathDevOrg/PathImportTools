...
describe("productCopy", () => {
  ...
  test("keeps the page focused on the import workflow", () => {
    ...
    expect(productCopy.heroBody).toBe("Select a backup folder. Path Import creates a database and downloads it automatically.");
    ...
    expect(appSource).toContain("shouldAskForSaveLocationRef.current = false");
    expect(appSource).toContain("privacy-footer");
