...
describe("Aura schema metadata", () => {
  test("describes one directly installable V12 schema", () => {
    ...
    expect(schemaFile).toEqual({ version: 12, name: "V12_schema.sql" });
  });

  test("builds a valid SQLite database directly from the V12 schema", () => {
  ...
  test("contains creation-only V12 DDL without the removed samples R-Tree", () => {
    ...
    expect(schemaSql).toContain("PRAGMA user_version = 12");
  });
